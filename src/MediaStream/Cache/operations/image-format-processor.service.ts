...
export class ImageFormatProcessor {
	...
	async processSvg(tempPath: string, resizeOptions: ResizeOptions, tenantSchema: string = 'public'): Promise<ProcessedImage> {
		...
		if (!svgContent.toLowerCase().includes('<svg')) {
			...
			return await this.processDefault(resizeOptions, tenantSchema)
		}
		...
		if (!needsResizing) {
			...
			return { data, metadata: this.buildMetadata(data.length.toString(), 'svg', tenantSchema) }
		}
		else {
	...
	async processRaster(tempPath: string, resizeOptions: ResizeOptions, tenantSchema: string = 'public'): Promise<ProcessedImage> {
		const result = await this.webpImageManipulationJob.handle(tempPath, resizeOptions)
	...
	async processDefault(resizeOptions: ResizeOptions, tenantSchema: string = 'public'): Promise<ProcessedImage> {
		const data = await this.optimizeAndServeDefaultImage(resizeOptions)
		return { data, metadata: this.buildMetadata(data.length.toString(), 'webp', tenantSchema) }
	}
