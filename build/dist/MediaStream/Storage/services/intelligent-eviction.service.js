...
var IntelligentEvictionService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.IntelligentEvictionService = void 0;
...
let IntelligentEvictionService = IntelligentEvictionService_1 = class IntelligentEvictionService {
    ...
    async performEviction(targetSize) {
        ...
        try {
            logger_util_1.CorrelatedLogger.debug(`Starting intelligent eviction with strategy: ${this.config.strategy}`, IntelligentEvictionService_1.name);
            ...
            logger_util_1.CorrelatedLogger.log(`Eviction completed: ${result.filesEvicted} files, ${this.formatBytes(result.sizeFreed)} freed`, IntelligentEvictionService_1.name);
            return result;
        }
        catch (error) {
            logger_util_1.CorrelatedLogger.error(`Eviction failed: ${error.message}`, error.stack, IntelligentEvictionService_1.name);
            return {
                filesEvicted: 0,
                sizeFreed: 0,
    ...
    async evictFiles(candidates) {
        ...
        for (const candidate of candidates) {
            try {
                const filePath = (0, node_path_1.join)(this.storageDirectory, candidate.file);
                await node_fs_1.promises.unlink(filePath);
                filesEvicted++;
                sizeFreed += candidate.size;
                logger_util_1.CorrelatedLogger.debug(`Evicted file: ${candidate.file} (${this.formatBytes(candidate.size)})`, IntelligentEvictionService_1.name);
            }
            catch (error) {
                const errorMsg = `Failed to evict ${candidate.file}: ${error.message}`;
                errors.push(errorMsg);
                logger_util_1.CorrelatedLogger.warn(errorMsg, IntelligentEvictionService_1.name);
            }
        }
        return {
            filesEvicted,
...
exports.IntelligentEvictionService = IntelligentEvictionService;
exports.IntelligentEvictionService = IntelligentEvictionService = IntelligentEvictionService_1 = __decorate([
