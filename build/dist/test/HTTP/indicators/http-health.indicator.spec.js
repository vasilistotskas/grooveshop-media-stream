...
describe('httpHealthIndicator', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        mockConfigService.getOptional.mockImplementation((key, defaultValue)=>{
            if (key === 'http.healthCheck.urls') return [];
            if (key === 'http.healthCheck.timeout') return 5000;
            return defaultValue;
        });
    ...
    describe('health Check - With URLs Configured', ()=>{
        ...
        it('should return healthy when all endpoints are healthy and circuit is closed', async ()=>{
            ...
            expect(result.http.checks.every((check)=>check.success)).toBe(true);
        });
        it('should return unhealthy when some endpoints fail', async ()=>{
            ...
            mockHttpClientService.get.mockResolvedValueOnce(mockSuccessResponse).mockRejectedValueOnce(new Error('Connection failed'));
            const result = await indicator.isHealthy();
            expect(result.http.status).toBe('down');
        ...
        it('should include response times for successful requests', async ()=>{
            ...
            const result = await indicator.isHealthy();
            expect(result.http.status).toBe('up');
            expect(result.http.checks.every((check)=>check.responseTime > 0)).toBe(true);
        });
    });
    describe('error Handling', ()=>{
        ...
        it('should handle unexpected errors gracefully', async ()=>{
            ...
            const testIndicator = module.get(HttpHealthIndicator);
            mockHttpClientService.isCircuitOpen.mockReturnValue(false);
            mockHttpClientService.getStats.mockReturnValue({
                totalRequests: 0,
...
//# sourceMappingURL=http-health.indicator.spec.js.map