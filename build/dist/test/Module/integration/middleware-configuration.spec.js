...
describe('middleware Configuration', ()=>{
    let app;
    let module;
    let _correlationService;
    let metricsService;
    // Helper function for reliable HTTP requests
    const makeReliableRequest = async (path, options = {})=>{
        const maxRetries = 3;
        let lastError;
        for(let attempt = 1; attempt <= maxRetries; attempt++){
            try {
                const req = request(app.getHttpServer()).get(path).timeout(8000);
    ...
    beforeAll(async ()=>{
        module = await Test.createTestingModule({
        ...
        }).overrideProvider(CacheHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                cache: {
                    status: 'up'
                }
            })
        }).overrideProvider(RedisHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                redis: {
                    status: 'up'
                }
            })
        }).overrideProvider(DiskSpaceHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                disk: {
                    status: 'up',
                    free: '10GB',
            ...
            getCurrentDiskInfo: vi.fn().mockResolvedValue({
                free: '10GB',
                used: '5GB',
                total: '15GB'
            })
            ...
            getCurrentMemoryInfo: vi.fn().mockReturnValue({
                used: '100MB',
                free: '900MB',
                total: '1GB'
            })
        }).overrideProvider(HttpHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                http: {
                    status: 'up'
                }
            })
        }).overrideProvider(AlertingHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                alerting: {
                    status: 'up'
                }
            })
        }).overrideProvider(SystemHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                system: {
                    status: 'up'
                }
            })
        }).overrideProvider(JobQueueHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                jobQueue: {
                    status: 'up'
                }
            })
        }).overrideProvider(StorageHealthIndicator).useValue({
            isHealthy: vi.fn().mockResolvedValue({
                storage: {
                    status: 'up'
                }
    ...
    describe('middleware Chain Order', ()=>{
        ...
        it('should handle middleware errors gracefully', async ()=>{
            ...
            const response = await request(app.getHttpServer()).get('/nonexistent-route').expect(404);
    ...
    describe('global Guards', ()=>{
        it('should apply rate limiting', async ()=>{
            ...
            for(let i = 0; i < requestCount; i++){
                try {
                    const response = await request(app.getHttpServer()).get('/health').timeout(8000).retry(2) // Add retry for network issues
                    ;
    ...
    describe('global Exception Filter', ()=>{
        it('should handle exceptions with correlation ID', async ()=>{
            ...
            const response = await request(app.getHttpServer()).get('/nonexistent-route').set('x-correlation-id', correlationId).expect(404);
    ...
    describe('integration with Health Checks', ()=>{
        it('should allow health checks to bypass rate limiting', async ()=>{
            ...
            try {
                ...
                const responses = [];
                for(let i = 0; i < requestCount; i++){
                    const response = await request(app.getHttpServer()).get('/health').timeout(5000);
                    responses.push(response);
...
//# sourceMappingURL=middleware-configuration.spec.js.map