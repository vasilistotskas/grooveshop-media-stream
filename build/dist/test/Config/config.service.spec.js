...
describe('configService', ()=>{
    ...
    describe('configuration Loading', ()=>{
        ...
        it('should get optional configuration values with defaults', ()=>{
            ...
            expect(service.getOptional('nonexistent.key', 'default')).toBe('default');
        });
        ...
        it('should throw error for non-existent configuration key', ()=>{
            expect(()=>service.get('nonexistent.key')).toThrow('Configuration key \'nonexistent.key\' not found');
        });
    });
    describe('configuration Validation', ()=>{
        it('should validate server configuration', async ()=>{
            nestConfigService.get.mockImplementation((key)=>{
        ...
        it('should validate cache configuration', async ()=>{
            nestConfigService.get.mockImplementation((key)=>{
                if (key === 'CACHE_MEMORY_MAX_SIZE') return '-1';
                return mockEnvVars[key];
            });
            const invalidService = new ConfigService(nestConfigService);
            await expect(invalidService.validate()).rejects.toThrow('Configuration validation failed');
        ...
        it('should validate processing configuration', async ()=>{
            nestConfigService.get.mockImplementation((key)=>{
                if (key === 'PROCESSING_MAX_CONCURRENT') return '0';
                return mockEnvVars[key];
            });
            const invalidService = new ConfigService(nestConfigService);
            await expect(invalidService.validate()).rejects.toThrow('Configuration validation failed');
        ...
        it('should validate external services URLs', async ()=>{
            nestConfigService.get.mockImplementation((key)=>{
                if (key === 'NEST_PUBLIC_DJANGO_URL') return 'invalid://url with spaces';
                return mockEnvVars[key];
            });
            const invalidService = new ConfigService(nestConfigService);
            await expect(invalidService.validate()).rejects.toThrow('Configuration validation failed');
    ...
    describe('default Values', ()=>{
        it('should use default values when environment variables are not set', async ()=>{
            ...
            const serviceWithDefaults = new ConfigService(emptyNestConfigService);
    ...
    describe('type Safety', ()=>{
        it('should maintain type safety for configuration values', ()=>{
            ...
            const enabled = service.get('monitoring.enabled');
            const formats = service.get('processing.allowedFormats');
