...
import { RedisCacheService } from "../../../MediaStream/Cache/services/redis-cache.service.js";
...
describe('redisHealthIndicator', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        const mockConfigService = {
            get: vi.fn().mockImplementation((key)=>{
                if (key === 'cache.redis.host') return mockConfig.host;
                if (key === 'cache.redis.port') return mockConfig.port;
                if (key === 'cache.redis.db') return mockConfig.db;
                if (key === 'cache.redis.ttl') return mockConfig.ttl;
                if (key === 'cache.redis.maxRetries') return mockConfig.maxRetries;
                return undefined;
            })
        };
    ...
    describe('performHealthCheck', ()=>{
        ...
        it('should return unhealthy status for slow response times', async ()=>{
            ...
            redisCacheService.ping.mockImplementation(()=>new Promise((resolve)=>setTimeout(()=>resolve('PONG'), 250)));
            redisCacheService.set.mockResolvedValue(undefined);
        ...
        it('should generate warnings for performance issues', async ()=>{
            ...
            const result = await indicator.isHealthy();
            expect(result.redis.warnings).toEqual(expect.arrayContaining([
...
//# sourceMappingURL=redis-health.indicator.spec.js.map