...
import { StorageCleanupService } from "../../../MediaStream/Storage/services/storage-cleanup.service.js";
...
describe('storageHealthIndicator', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        indicator = module.get(StorageHealthIndicator);
    ...
    describe('isHealthy', ()=>{
        ...
        it('should return unhealthy status when storage exceeds thresholds', async ()=>{
            storageMonitoring.checkThresholds.mockResolvedValue({
                status: 'critical',
                issues: [
                    'Storage size critical: 1.2GB exceeds 1GB limit'
                ],
                stats: mockStorageStats
            });
            const result = await indicator.isHealthy();
        ...
        it('should include recommendations when available', async ()=>{
            ...
            const result = await indicator.isHealthy();
            expect(result.storage.recommendations).toContain('Schedule cleanup soon to prevent storage issues');
        });
    ...
    describe('configuration-based recommendations', ()=>{
        ...
        it('should recommend cleanup when disabled and storage is full', async ()=>{
            ...
            const result = await indicator.isHealthy();
            expect(result.storage.recommendations).toContain('Enable automatic cleanup to maintain storage health');
        });
        it('should recommend file review when cleanup is stale', async ()=>{
            ...
            const result = await indicator.isHealthy();
            expect(result.storage.recommendations).toContain('Last cleanup was over a week ago - consider running manual cleanup');
        });
...
//# sourceMappingURL=storage-health.indicator.spec.js.map