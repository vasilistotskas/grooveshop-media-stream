import { BullQueueService } from "../../../MediaStream/Queue/services/bull-queue.service.js";
import { JobType } from "../../../MediaStream/Queue/types/job.types.js";
...
describe('bullQueueService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(BullQueueService);
    ...
    describe('add', ()=>{
        it('should add job to image processing queue', async ()=>{
            ...
            expect(mockImageQueue.add).toHaveBeenCalledWith(JobType.IMAGE_PROCESSING, jobData, expect.objectContaining({
                priority: 1,
                attempts: 3,
                backoff: {
                    type: 'exponential',
        ...
        it('should add job to cache operations queue', async ()=>{
            ...
            expect(mockCacheQueue.add).toHaveBeenCalledWith('cache-warming', jobData, expect.objectContaining({
                delay: 5000,
                attempts: 3,
                timeout: 30000
            }));
        ...
        it('should handle job addition errors', async ()=>{
            ...
            await expect(service.add(JobType.IMAGE_PROCESSING, jobData)).rejects.toThrow('Queue connection failed');
        });
    });
    describe('process', ()=>{
        it('should register job processor', ()=>{
            ...
            expect(mockImageQueue.process).toHaveBeenCalledWith(JobType.IMAGE_PROCESSING, expect.any(Function));
        });
    ...
    describe('getStats', ()=>{
        it('should return combined queue statistics', async ()=>{
            ...
            mockImageQueue.getWaiting.mockResolvedValue([
                {},
                {}
            ]); // 2 waiting
            mockImageQueue.getActive.mockResolvedValue([
                {}
            ]); // 1 active
            mockImageQueue.getCompleted.mockResolvedValue([
                {},
                {},
                {}
            ]); // 3 completed
            mockImageQueue.getFailed.mockResolvedValue([
                {}
            ]); // 1 failed
            ...
            mockCacheQueue.getWaiting.mockResolvedValue([
                {}
            ]); // 1 waiting
            mockCacheQueue.getActive.mockResolvedValue([]); // 0 active
            mockCacheQueue.getCompleted.mockResolvedValue([
                {},
                {}
            ]); // 2 completed
            mockCacheQueue.getFailed.mockResolvedValue([]); // 0 failed
            mockCacheQueue.getDelayed.mockResolvedValue([
                {}
            ]); // 1 delayed
    ...
    describe('removeJob', ()=>{
        ...
        it('should throw error when job not found', async ()=>{
            ...
            await expect(service.removeJob('nonexistent-job')).rejects.toThrow('Job nonexistent-job not found');
        });
    ...
    describe('clean', ()=>{
        it('should clean jobs from all queues', async ()=>{
            ...
            await service.clean(grace, status);
        ...
        it('should handle clean errors', async ()=>{
            ...
            await expect(service.clean(grace, status)).rejects.toThrow('Clean failed');
        });
    });
    ...
    describe('convertToBullOptions', ()=>{
        it('should convert job options correctly', ()=>{
            ...
            const bullOptions = service.convertToBullOptions(options);
...
//# sourceMappingURL=bull-queue.service.spec.js.map